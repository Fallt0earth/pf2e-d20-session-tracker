...
**Release status.** **v1.0.0 released 2026-09-19** from `main` at https://github.com/Fallt0earth/pf2e-d20-session-tracker (tag `v1.0.0`; the Release workflow attached `module.json` and `module.zip`, verified by download: 79 files, manifest at the top level, download pinned to the tag). Install manifest: `https://github.com/Fallt0earth/pf2e-d20-session-tracker/releases/latest/download/module.json`. The intermediate tags planned for M1–M3 (`v0.1.0`–`v0.3.0`) were never cut; 1.0.0 is the first release. **v1.1.0 released 2026-09-19** (M5 session definition + the GM "Players see" control; release assets verified by download: manifest 1.1.0, 80 files). Licence stays GPL-3.0 (confirmed by David). **2026-09-21:** the GitHub repository was deleted and recreated (D9) and `main`, `v1.0.0` and `v1.1.0` were pushed again from the rewritten history; it is **private** until David flips it, and the Forge needs it public to fetch the manifest. **1.1.1 (M6: code hardening, D10, and the supply-chain pass, D11) was committed (`5cce704`), tagged `v1.1.1` and pushed on 2026-09-21** on David's word; it is the version to install for the Forge gates. The tag started the rewritten Release workflow for the first time; **its run and the release assets could not be checked from here while the repository is private** (no API access is used beyond git). Next: David checks the Actions run (or makes the repo public, after which the manifest and zip are verified by download as for 1.0.0 and 1.1.0), runs the verified Node 24 installer (D11), then the Forge gates (manual, David), then M4 when the Forge game moves to v14, then the post-1.0 backlog. If the Release run failed, nothing was published under 1.1.1: fix the workflow and cut 1.1.2 rather than moving the tag.
...
- [ ] David: run `Downloads\node-v24.21.0-x64.msi` (verified; needs the UAC approval), then `node --version` should say v24.21.0. Afterwards bump `playwright-core` deliberately (edit the exact version and the allow-list stays as it is).
...
- [ ] Confirm the Release run (first run of the rewritten workflow) and the 1.1.1 assets: `module.json` 1.1.1, `module.zip`, `SHA256SUMS.txt`; `releases/latest/download/module.json` must report 1.1.1. Needs the repo public, or David's look at the Actions tab.
