...
**Release status.** **v1.0.0 released 2026-09-19** from `main` at https://github.com/Fallt0earth/pf2e-d20-session-tracker (tag `v1.0.0`; the Release workflow attached `module.json` and `module.zip`, verified by download: 79 files, manifest at the top level, download pinned to the tag). Install manifest: `https://github.com/Fallt0earth/pf2e-d20-session-tracker/releases/latest/download/module.json`. The intermediate tags planned for M1–M3 (`v0.1.0`–`v0.3.0`) were never cut; 1.0.0 is the first release. **v1.1.0 released 2026-09-19** (M5 session definition + the GM "Players see" control; release assets verified by download: manifest 1.1.0, 80 files). Licence stays GPL-3.0 (confirmed by David). **2026-09-21:** the GitHub repository was deleted and recreated (D9) and `main`, `v1.0.0` and `v1.1.0` were pushed again from the rewritten history; it is **private** until David flips it, and the Forge needs it public to fetch the manifest. **1.1.1 (M6: code hardening, D10, and the supply-chain pass, D11) is built and verified on the dev instance but not yet committed, tagged or pushed**; it is the version to install for the Forge gates. Next: commit + tag `v1.1.1` + push (on David's word), make the repo public, the Forge gates (manual, David), then M4 when the Forge game moves to v14, then the post-1.0 backlog.
...
- [ ] Commit, tag `v1.1.1`, push; confirm the pinned-action Release run and the 1.1.1 manifest (David's word needed; the repo is private until he flips it).
