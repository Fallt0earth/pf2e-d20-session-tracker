...
**Release status.** **v1.0.0 released 2026-09-19** from `main` at https://github.com/Fallt0earth/pf2e-d20-session-tracker (tag `v1.0.0`; the Release workflow attached `module.json` and `module.zip`, verified by download: 79 files, manifest at the top level, download pinned to the tag). Install manifest: `https://github.com/Fallt0earth/pf2e-d20-session-tracker/releases/latest/download/module.json`. The intermediate tags planned for M1–M3 (`v0.1.0`–`v0.3.0`) were never cut; 1.0.0 is the first release. **v1.1.0 released 2026-09-19** (M5 session definition + the GM "Players see" control; release assets verified by download: manifest 1.1.0, 80 files). Licence stays GPL-3.0 (confirmed by David). **2026-09-21:** the GitHub repository was deleted and recreated (D9) and `main`, `v1.0.0` and `v1.1.0` were pushed again from the rewritten history; it is **private** until David flips it, and the Forge needs it public to fetch the manifest. **1.1.1 (M6: code hardening, D10, and the supply-chain pass, D11) was committed (`5cce704`), tagged `v1.1.1` and pushed on 2026-09-21** on David's word; it is the version to install for the Forge gates. **v1.1.1 released and verified 2026-09-22** after David made the repository public: the first run of the rewritten Release workflow succeeded (as did the re-runs for v1.0.0 and v1.1.0 on the recreated repo); GitHub's "latest" is v1.1.1; `releases/latest/download/module.json` reports 1.1.1 with `download` pinned to `v1.1.1/module.zip`; `module.zip` (137 978 B, 68 files) and `module.json` match `SHA256SUMS.txt`; every file in the zip is byte-identical to the git tree at the tag (the jq re-stamp reproduced `module.json` exactly); nothing from `dev/`, `test/` or `docs/` ships; the removed local-only files 404 at `main` and all three tags; an anonymous clone (15 commits) scans clean for host details and the pre-rewrite commits are unreachable. Node 24 is installed (D11). **v1.2.0 tagged 2026-09-22** on David's word ("push and tag"): the Node 24 baseline made enforceable, `playwright-core` 1.63.0, and the resource pass (M7: History tab quadratic → linear, one roll = one key on the wire, render debounce, catch-up window). Release run success; verified by download (M7 checklist). **v1.3.0 tagged 2026-09-22** on David's word ("push and tag"): the whole-history backfill (M8). **Install 1.3.0 on the Forge**; release verification in the M8 checklist. Next: the Forge gates (manual, David: install from the manifest, one real evening, idle/wake, player walkthrough), then M4 when the Forge game moves to v14, then the post-1.0 backlog.
...
- **D9 — Public-repo hygiene (2026-09-21).** A security review found that internal host documentation had been committed to this public repository: the NAS quick-reference, another project's deploy script, and host names, addresses, the SSH login and a port inventory inside this plan and the dev scripts. No credential was ever committed (full-history scan: only Foundry vocabulary and the placeholder env example matched). History was rewritten the same day: those two files were removed from every commit, identifiers were replaced throughout, both release tags were re-pointed, and `main` was force-pushed. Host details now live only in git-ignored `dev/local.ps1` and `dev/local.json` (templates: `dev/local.example.*`); `docs/nas-quickstart.MD` and `dev/reference/` stay on the owner's machine, git-ignored. Rule, also in `CLAUDE.md`: read `git status` before every commit and never `git add -A` blindly.
- **D11 — Supply chain: as little as possible (David, 2026-09-21).** Inventory before: 90 installed packages (ESLint 86, esbuild 2, Playwright 2), one install script (esbuild), ESLint 9 end-of-life with no upgrade path on Node 18, `npm audit signatures` unusable on npm 9.5.1 (it predates a registry key rotation); no locked version matched a known-compromised release; the shipped module never contained or loaded third-party code. Policy now: **(1)** nothing from npm ships, and the one build artifact (`macros/analyze.js`) is verified against its sources; **(2)** the dev tree is two exact-pinned, dependency-free packages from one publisher (`typescript` 6.0.3, the last pure-JavaScript line; `playwright-core` 1.55.1, the last line for Node 18), with `ignore-scripts=true` and `npm ci`; **(3)** CI uses only `actions/checkout` pinned to a commit, creates the release with the runner's `gh`, never installs packages, publishes `SHA256SUMS.txt`; **(4)** `test/supply-chain.test.js` turns all of it into assertions. **Node (David: "you can update node", 2026-09-21):** target is the Active LTS, v24.21.0 (npm 11.19). The official MSI was downloaded from nodejs.org and verified twice (SHA-256 against `SHASUMS256.txt`; Authenticode: valid, signer OpenJS Foundation, Microsoft timestamp), but the machine-wide install needs a UAC approval that did not come through from the background session; **David ran the installer himself (2026-09-22): the PC now runs v24.21.0 / npm 11.19.0**, the installed `node.exe` hashes to the published `SHASUMS256` entry and carries a valid OpenJS Foundation signature, and the installer's optional "tools" step added nothing. The project is already validated on a verified temporary copy of 24.21.0: `npm ci`, `npm audit signatures` (2 verified registry signatures, 1 verified provenance attestation: `playwright-core`), `npm audit` 0, lint, 107 tests, a byte-identical macro build, and the e2e smoke. `engines.node` was `>=22` for 1.1.1 and is `>=24` from 1.2.0 on (enforced: `engine-strict=true`, probed 2026-09-22: without it npm only warns and installs), `.node-version` says 24, and `.npmrc` carries `min-release-age=14`. With Node 24 the default, `playwright-core` moved 1.55.1 → 1.63.0 (2026-09-22; 17 days public, so it passed the 14-day cooldown; no dependencies, no install scripts, verified registry signature and SLSA provenance attestation; the e2e smoke and `verify-hardening` 12/12 pass on it). Owner-side and outside the repo: 2FA on GitHub and npm-free publishing (nothing is published to npm), a tag ruleset for `v*` plus immutable releases, Dependabot alerts.
- **D10 — Trust model (2026-09-21).** Foundry rolls dice on the roller's client and lets a user write any flags, speaker, content and timestamp on their own messages, so the module cannot prove a die was rolled and does not try. What it guarantees instead: **a player's messages cannot change anybody else's numbers, cannot grow the log without bound, and cannot stall or break the GM's client.** The GM (writer) and the server-enforced `message.author` are the only trusted inputs; everything else in a message is validated in the pure layer (`capture/sanitize.js`). Accepted by design: a player can invent their own dice; blind results are readable through the console (README); grouping by character follows the message's speaker; the dev instance has no user passwords (LAN/Tailscale only, David's call).
...
├─ macros/
│  ├─ analyze.js                  M0 — GENERATED single-file bundle (dev/build-macro.mjs) of the pure layer + a console/chat report
...
│  │  ├─ html-scan.js             pure: clipped, linear-time HTML scanning shared by reroll-html and flat-check (M6)
│  │  ├─ sanitize.js              pure: shapes and bounds for everything read from a message; sanitizeRecord (M6, D10)
...
│  ├─ backfill/
│  │  ├─ catchup.js               the running session's game.messages → normalize → store (on GM ready + a button)
│  │  ├─ plan.js                  pure: whole-history plan (range, one Sessionizer, per-evening counts, junk-evening ticks) — M8
│  │  └─ backfill.js              Foundry: chunked read of game.messages with progress, one page write per evening — M8
│  ├─ sessions/
...
│  │  ├─ merge.js                 pure: what may change when a record with a stored id arrives again (M6)
...
│  │  ├─ backfill-app.js          the backfill dialog (range, knobs, preview with ticks, progress, summary) — M8
...
├─ dev/                           NOT shipped: docker-compose.yml, foundry-dev.env.example, deploy.ps1, backup.ps1, nas-README.md (recovery notes copied to the NAS), anonymize-fixtures.mjs, build-macro.mjs, lint.mjs + lint/ (D11), e2e/ (playwright-core harness)
...
- **`macros/analyze.js` is built by `dev/build-macro.mjs` on the TypeScript compiler** (D11; esbuild until 1.1.0): each module becomes a CommonJS function in one registry, only relative imports under `scripts/` are followed, and `test/macro-bundle.test.js` checks that the committed bundle is current and behaves like its sources.
- **Tests: `node --test`** (Node 20+ ships full ICU, so `Intl` timezone tests need nothing extra). No test-framework dependency.
- **Lint: `dev/lint.mjs` on the TypeScript compiler** (D11): undefined names, missing imports and exports, redeclarations, use before declaration and syntax are errors; unused locals are warnings; type-level findings are ignored. The pure layer is compiled in a program of its own with no Foundry globals and no DOM (`dev/lint/globals-*.d.ts`), and nothing outside it may be reachable from it, so the purity rule holds by construction. The pure file list lives in `dev/lint.mjs`. *Until 1.1.0:* ESLint flat config, `no-restricted-globals` (`game`, `foundry`, `Hooks`, `Roll`, `ui`, `canvas`, `document`, `window`) and `no-restricted-imports` scoped to `scripts/stats/**`, `scripts/capture/normalize.js`, `scripts/capture/extractors/**`, `scripts/capture/dice-walk.js`, `scripts/capture/reroll-html.js`, `scripts/sessions/bucket.js`. This enforces SCOPE's "keep the stats layer free of Foundry globals" mechanically.
...
- Writes go through a serial queue (`util/queue.js`) with a 250 ms debounce, so bursts (initiative for eight tokens, "Roll NPC Saves") become one `updateEmbeddedDocuments` call. **Since 1.2.0 (M7) the rows are keyed by record id** (`data.rows = { [id]: row }`, codec v2): Foundry replaces arrays whole but merges objects key by key, so `journal.writePage` sends the smallest update that makes the page match (`codec.flagsUpdate`: new and changed rows by key, removed rows as `-=id`, meta likewise) and one roll is ~760 B on the wire for every client instead of the whole page (measured: 80 KB at 300 dice). v1 pages decode unchanged and are rewritten in v2 on their next write (verified live on a planted page). Backfill writes in chunks of at most 20 pages or ~2 MB per call.
...
### M6 — Hardening from the security review (1.1.1) — built and verified 2026-09-21, not yet released
Source: the static security review of 2026-09-21 (D9 covered the repository itself; this covers the code). Trust model: D10.
- [x] `capture/sanitize.js` (pure): document-id, natural, slug and token shapes; `sanitizeRecord` coerces every record to the stored shape (bounded strings, enumerated outcome / mode / rollTwice, finite numbers, id must start with its message id). Applied to every record in `normalize.js`.
- [x] Normalizer: an extractor that throws loses its own message only (a malformed message used to abort a whole catch-up); 24 dice per message; `effectiveTimestamp`: nothing is filed in the future, and a **player's live** roll more than 15 min from the writer's clock gets the writer's time (GM messages keep theirs, so imports and `verify-m5` still work; catch-up keeps past timestamps).
- [x] Dice walk: d20 results must be 1–20; depth 12, 2 000 terms, 50 rolls, 200 KB of roll JSON.
- [x] Reroll annotation (`flags[MODULE_ID].reroll`): id shape, naturals 1–20, must agree with the roll in the message (the message carries the kept roll), else the HTML fallback runs. It may refresh only the **reroll link** (kept / rerolledBy / rerollOutcome / resource) of the **roller's own** stored original, or of any original when a GM rerolls; a stored original by another user, or a live message by another user under that id, is left alone. New ctx members: `find`, `isGM`, `messageAuthor`, `now`, `warn`.
- [x] `storage/merge.js` (pure) + `store.append`: a stored record's natural, ts, msgId, sessionKey and non-guessed userId never change through an upsert (defence in depth behind the extractor rule). 5 000 records per session, one warning per session (`PF2E-D20.Sessions.Full`). Journal pages whose key is not a session key are ignored.
- [x] Toolbelt saves: save keys must be token ids; `rollerId` is believed only when it is a user id and, when the token resolves, that user is an owner of the actor or a GM (`resolveToken().rollers`), else updater + `userGuess`; 64 targets × 8 variants; re-rolls of one save stop getting records after sequence 4; impossible dice are skipped.
- [x] `capture/html-scan.js` (pure): clipped input (50 KB), collapsed whitespace, bounded quantifiers; `reroll-html.js` and `flat-check.js` rebuilt on it (the old patterns backtracked polynomially on crafted content). Fixture tests unchanged and green.
- [x] Prototype-safe tallies: Maps in `summarize.js` / `view-model.js`, prototype-free `cells` in `history-model.js`, own-property label lookups (view-model, csv), codec decodes known fields only, analyzer macro tallies.
- [x] Report: `isSessionKey`; `openReport` needs a well-formed key of a stored session; link button wired only on a GM-authored message carrying the module flag; key escaped in the posted HTML; window element id sanitized.
- [x] CSV: text cells starting with `= + - @` (or tab / CR) get a leading apostrophe; numbers untouched. Analyzer whisper escapes everything.
- [x] Release workflow: `actions/checkout` pinned to a commit (v4.4.0, where the floating `v4` pointed on 2026-09-21) with `persist-credentials: false`; `softprops/action-gh-release` replaced by `gh release create --verify-tag --generate-notes` on the runner; default token permission none, `contents: write` on the one job; tag validated as `vX.Y.Z` and passed through `env`, never interpolated into a script; `SHA256SUMS.txt` attached. **First exercised by the `v1.1.1` tag.**
- [x] Supply-chain pass (D11): dev tree 90 → 2 packages (`typescript` 6.0.3, `playwright-core` 1.55.1; exact, no dependencies, no install scripts), `.npmrc` (`ignore-scripts`, `save-exact`, registry), `dev/lint.mjs` replaces ESLint (probed with planted violations: undefined names, DOM / Node / Foundry names in the pure layer, a pure file reaching a Foundry-facing one, missing imports and exports), `dev/build-macro.mjs` replaces esbuild (old and new bundle produce identical results, console output and whisper HTML over the fixture corpus), `test/supply-chain.test.js` (5; all 13 planted policy violations caught) and `test/macro-bundle.test.js` (3). `npm ci` from the lockfile reproduces the tree; `npm audit`: 0.
- [x] Tests: `test/hardening.test.js` (21), `supply-chain` (5), `macro-bundle` (3) → 107 unit tests; lint clean with the new pure files (and `sessionizer.js`) under the purity rule. Dev instance, 2026-09-21: `verify-m2` 18/18, `verify-m5` 16/16, new `verify-hardening` 12/12 (docs/TESTING.md; re-run on the `playwright-core` harness with the rebuilt macro), stored sessions identical before and after.
- [x] Node 24 readiness: `npm test` is now `node --test` (the old `node --test test/` runs **nothing** on Node 22+, where a directory argument is executed as a file; default discovery finds the same 107 tests on 18.16 and 24.21), `engines.node >=22`, `.node-version`, `.npmrc min-release-age=14` (known to npm 11.19; ignored by npm 9), everything re-validated on a verified temporary Node 24.21.0 (D11).
- [x] Node 24.21.0 installed by David (2026-09-22); verified binary; full gate green on it (`npm ci`, `npm audit signatures` 2 verified + 1 attestation, audit 0, lint 0/0, 107 tests, byte-identical macro build). `playwright-core` bumped 1.55.1 → 1.63.0 under the cooldown; smoke + `verify-hardening` 12/12 on the dev instance. The temporary validation copy of Node was removed; the verified MSI stays in `Downloads`.
- [x] Commit (`5cce704`), tag `v1.1.1`, push: 2026-09-21. Staged by explicit path; every added line scanned for host details, keys, tokens, local paths and e-mail addresses (none).
- [x] Release run confirmed 2026-09-22 (repo public): success; assets `module.json` (1.1.1), `module.zip`, `SHA256SUMS.txt`; checksums verified by download; zip byte-identical to the tagged tree; latest manifest reports 1.1.1.

### M7 — Resource pass (1.2.0) — built and verified 2026-09-22, tagged v1.2.0 the same day (David: "make sure the code itself won't eat too many resources")
Measured first (`dev/e2e/measure-resources.mjs` on the dev instance; a node benchmark of the pure layer), then fixed what the numbers justified. Budget after the pass, per client, from the measurements in docs/TESTING.md:
- [x] **History tab was quadratic** in the dice count: `binomialAtLeast/AtMost` summed the pmf term by term with an O(k) `logChoose` per term, so the all-time tails over years of dice cost 0.4 s (1 year), 3.8 s (3 years), 10.4 s (5 years) per model, recomputed per roll while the tab was open. Now the tails walk outwards from the boundary term with the neighbour ratio and stop at the double floor: 6 / 13 / 25 ms; agreement with the old sum 3e-12 over 650 000 cases; identities hold at n = 50 000; 0.02 ms per tail. The page summary at the 5 000 cap went 104 → 1.3 ms for the same reason.
- [x] **Per-roll broadcast was the whole page**: every client received the page's entire `data` array on every roll (measured 2 055 B for a 12-record page, i.e. ~80 KB at 300 dice, ~1.3 MB at the cap, times the number of connected clients). Rows keyed by id (codec v2, §4.5): 761 B per roll, flat. GM-side capture + upsert 0.1 ms, journal write round trip 9 ms, player decode 0.1 ms, player re-render 2.6 ms.
- [x] **Renders**: an open window redraws 300 ms after the last change (bursts become one render); the Fun (Monte Carlo) and History models are built only when that part is actually being drawn (the Fun model used to run its Monte Carlo on every roll for a part that was not redrawn), and the active Fun/History tab now stays current instead of going stale; the report popup memoizes its Monte Carlo and redraws on the same delay. Monte Carlo cost, 6 groups: 64 / 86 / 139 ms at 120 / 300 / 600 dice (10k iterations), 27 / 44 / 67 ms in the report (5k).
- [x] **Catch-up at GM start** ran `Intl` for every chat message in the world (2.4 µs each: 120 ms per 50 000 messages); a coarse timestamp window (`bucket.dayWindow`, proven for every timezone and boundary hour in `test/bucket.test.js`) first: 1.2 ms.
- [x] Checked and left alone: capture 5 µs per message; Sessionizer 9 µs per call with 300 stored sessions; decode of all pages at load 19 / 56 / 99 ms for 1 / 3 / 5 years; heap 643 B per record (50 000 records = 31 MB, plus Foundry's own copy of the page flags at 268 B/record); at rest 2.6 / 7.7 / 12.8 MB in the world for 1 / 3 / 5 years, which every client downloads at join (Foundry sends the whole world); no timers, no animations, no polling. Nothing here needs a fix before years of play; a page-archiving option (compress evenings older than a year) is the post-1.0 item if join size ever matters.
- [x] Tests 107 → 113 (codec v2 round trip, roll order, one roll = one key, deletions, v1 rewrite, meta deletions; binomial identities and speed; `dayWindow`). Dev instance: `measure-resources` before/after, `verify-m2` 18/18, `verify-m5` 16/16, `verify-hardening` 12/12, planted v1 page migrates on one write.
- [x] **Released as v1.2.0 (`6922afe`) on 2026-09-22** and verified by download: Release run success; latest manifest reports 1.2.0 with `download` pinned to `v1.2.0/module.zip`; `module.zip` (142 507 B, 68 files) and `module.json` match `SHA256SUMS.txt`; every file byte-identical to the tagged tree; nothing from `dev/`, `test/` or `docs/`. **This is the version for the Forge gates.**

### M8 — Whole-history backfill (1.3.0) — built and verified 2026-09-22, tagged v1.3.0 the same day (David: "yeah implement backfill")
SCOPE Should-6 and D5, the item parked on 2026-09-15. Same normalizer, same Sessionizer, same store; the new code is a pure planner and a dialog.
- [x] `backfill/plan.js` (pure): messages in a range, sorted, one Sessionizer seeded with the stored intervals, `event: "backfill"` (past timestamps kept, future ones capped), records the store already holds skipped and counted as `existing`, per-evening `n` / `players` / `gmPresent` / first–last; SCOPE's junk-evening mitigations as the default ticks (`n ≥ minRolls` and, when required, a GM message that evening — `peek` places a message without dice as its neighbours were); `unscheduled` never ticked. 6 unit tests: daily corpus (3 evenings, a sheet-testing day, a players-only evening), the two knobs, skip and idempotence, range bounds in the world timezone (`bucket.localMidnight`), by-pause overnight and two-games-a-day with stored intervals, junk input.
- [x] `backfill/backfill.js`: `backfillBounds` (oldest message → today), `backfillPlan` (`toObject` in chunks of 400 with a yield and progress), `backfillRun` (chosen evenings oldest first, `append` + `flush` per evening, progress). `api.backfill({ from, to, minRolls, requireGM, dryRun, keys })` and `api.openBackfill()`.
- [x] `ui/backfill-app.js` + `templates/tracker/backfill.hbs`: form (dates prefilled from the world, threshold from `minRollsToList`, GM-required on), scan progress, preview table with a tick per evening and the reason it is unticked, tick all / none, write progress, summary; manual-mode hint. `ApplicationV2` reserves `state` and `form`: the dialog uses `phase` and `inputs`.
- [x] Dev instance (`dev/e2e/verify-backfill.mjs`, 16/16): plants 149 dated messages with three authors while capture is paused, checks the dry run, both knobs, a one-day range, the write (108 rolls, three pages, right users and keys), idempotence (second run adds 0, skips 108), adding an unticked evening by name, and the dialog itself (prefill, preview, tick none/all, add, close); stored sessions identical before and after. Regression: `verify-m5` 16/16, `verify-hardening` 12/12.
- [x] **Released as v1.3.0 (`4c7fe65`) on 2026-09-22** and verified by download: Release run success; latest manifest reports 1.3.0 with `download` pinned to `v1.3.0/module.zip`; `module.zip` (152 887 B, 72 files) and `module.json` match `SHA256SUMS.txt`; every file byte-identical to the tagged tree; nothing from `dev/`, `test/` or `docs/`. **This is the version for the Forge gates.**
- [ ] Forge: run it once over the campaign after the Forge gates (a world export first; it adds only, but the export is the way back for anything else).

### Post-1.0 backlog (deferred 2026-09-15)
- ~~**Whole-history backfill**~~ — done in M8 (1.3.0). Originally: the normalizer already accepts `event: "backfill"`; deferred were the all-sessions UI with progress and date range, the `gmPresent` heuristic and auto-exclusion of junk evenings, attribution guesses for Toolbelt saves and flat checks in old messages, and HTML-only recovery of reroll discards. Needs a world export and `dev/backup.ps1` before the first run on the Forge.
