...
Dev/test instance: http://your-docker-host:30000 (felddy Docker image on the NAS). Deploy with `.\dev\deploy.ps1` (sync only; `-Compose` after compose changes, `-Restart` to restart the container). Never hand-edit files under `/mnt/user/appdata/foundry-dev/app/`. Read docs/nas-quickstart.MD before any SSH or deploy step and follow its rules without exception: BatchMode ssh, no secrets in the repo or in output, no array/network/DNS actions. Never test against the Forge game except at the deploy gates in PLAN §5.
